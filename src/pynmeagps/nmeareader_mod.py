...
class NMEAReader:
    ...
    def parse(
        ...
        try:
            # split off checksum and check it
            ...
            if content[0:1] == "P": # proprietary message: starts with P and is in known list
                talker = "P"
                msgid = None
                for msgid_prop in nmt.NMEA_MSGIDS_PROP:
                    if content[1:].startswith(msgid_prop + ","):
                        msgid = msgid_prop
                        payload = content[len(msgid_prop)+2:]
                        break
                if msgid is None:
                    raise nme.NMEAMessageError(f"Unknown propietary message id {message}")
            else:  # standard messages format ttiii, (2x talker, msgid, comma)
